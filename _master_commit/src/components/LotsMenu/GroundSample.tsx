...
const GroundSample: React.FC<GroundSampleProps> = ({
  ...
  const steps = [t('laboratory'), t('soilCharacteristics'), t('variables'), t('attachments')];
  ...
  const getStepContent = (step: number) => {
    switch (step) {
      case 0:
        return <LaboratoryForm lot={lot} formData={formData} setFormData={setFormData} />;
      case 1:
        return <SoilCharacteristicsForm lot={lot} formData={formData} setFormData={setFormData} />;
      case 2:
        return <VariablesForm lot={lot} formData={formData} setFormData={setFormData} />;
      case 3:
        return <AttachmentsForm lot={lot} formData={formData} setFormData={setFormData} />;
      default:
  ...
  const handleSave = () => {
    for (let step = 0; step < steps.length; step++) {
      ...
      if (missingFields > 0) {
        setSnackbarMessage(t('completeRequiredFieldsInStep', { step: steps[step] }));
    ...
    let actividad = {
      ...
      actividad: {
        // Añadir estructura anidada que ActivityContent espera
        tipo: 'analisis_suelo',
        uuid: formData.id || uuidv4(),
        detalles: {
          hectareas: lot.properties.hectareas,
          fecha_ejecucion_tentativa: formData.fecha
      ...
      .catch(error => {
        if (error.name === 'not_found') {
          ...
          db.put(actividad)
            ...
            .catch(err => console.error('Error creating new actividad:', err));
      ...
      <Card className='shadow-lg'>
        ...
        >
          <Row className='align-items-center'>
            <Col>
              <h1 className='text-white mb-4' style={{ fontSize: '2rem', fontWeight: 'bold' }}>
                {t('soilSample')}
              </h1>
              ...
              <div className='d-flex gap-4'>
                ...
                <div className='d-flex align-items-center gap-2 bg-white bg-opacity-10 rounded-3 px-3 py-2'>
                  ...
                  <div>
                    ...
                    <div className='text-white fw-semibold'>{lot.properties.nombre}</div>
                  </div>
                </div>
              </div>
            </Col>
        ...
        {/* Content */}
        <CardBody className='p-4'>{getStepContent(activeStep)}</CardBody>
