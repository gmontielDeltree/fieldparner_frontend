...
export class ContratistaCrud extends LitElement {
  ...
  @property()
  contratista: Contratista = { labores:[], uuid:"", nombre:"", cuit:"", datos_generales: { email: "", direccion:"", telefono:"" } };
  ...
  agregar_labor(){
    ...
    // Immutable Way
    this.contratista = {...this.contratista, labores:[...this.contratista.labores, this._selected_labor]}
  }
  ...
  selected_item_changed(e: CustomEvent){
    this._selected_labor = e.detail.value ? e.detail.value : this._selected_labor;
  }
  /**
  ...
  private borrarRenderer = (root: HTMLElement, _: HTMLElement, model: any) => {
    ...
    const borrar_item = () => {
      let filtered = this.contratista.labores.filter((l) => l.uuid !== labor_item.uuid)
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              <vaadin-form-layout .responsiveSteps="${this.responsiveSteps}">
                <vaadin-text-field label="Nombre" .value=${this.contratista.nombre}
                @change=${(e: any) => {
                    this.contratista.nombre = e.target.value; 
                    this.contratista = {...this.contratista}
                    }}
                ></vaadin-text-field>
                <vaadin-text-field label="CUIT" 
                ...
                <vaadin-text-field label="Dirección" .value=${this.contratista.datos_generales.direccion}
                @change=${(e: any) => {
                ...
                <vaadin-text-field label="Teléfono" .value=${this.contratista.datos_generales.telefono}
                @change=${(e: any) => {
...
<div>
              <vaadin-grid
                .items="${(this.contratista.labores.length === 0) ? this.labores_empty : this.contratista.labores}"
                all-rows-visible
              >
                <vaadin-grid-column
                  header="Labores"
                  ...
                  auto-width
                ></vaadin-grid-column>
                <vaadin-grid-column header="" .renderer="${this.borrarRenderer}"></vaadin-grid-column>
