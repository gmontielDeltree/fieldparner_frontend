...
export class MagrisReporteOC extends LitElement {
  ...
  dibujarElMapa(repo: MagrisRecord[]) {
    ...
    let source: GeoJSONSource = this.map.getSource(
      "tolva_track"
    ) as GeoJSONSource;
    if (!source) {
      this.map.addSource("tolva_track", {
        type: "geojson",
        data: geojson,
      });
    } else {
      source = this.map.getSource("tolva_track") as GeoJSONSource;
      source.setData(geojson);
    }

    this.map.fitBounds(bbox(geojson),{padding:{top:45,left:400,bottom:45}})
  ...
  render() {
    let id_tolva = (this.id as string).split(":")[1];
