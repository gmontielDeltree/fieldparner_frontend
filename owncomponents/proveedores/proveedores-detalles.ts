...
import { listar_ejecuciones_por_depo } from "../depositos/depositos_funciones";
...
import { listar_transferencias } from "../depositos/transferencias_funciones";
import { calcular_stock } from "../depositos/stock_funciones";
import { map } from "lit/directives/map.js";
import { showNotification } from "../helpers/notificaciones";

@customElement("proveedores-detalles")
export class ProveedoresDetalles extends LitElement {
  @property()
  ...
  private proveedor: Proveedor = nuevo_proveedor();
  ...
  // Encadeno promises
  loadData(proveedor_uuid) {
    return cargar_proveedor(proveedor_uuid)
      .then((d) => (this.proveedor = d))
      .then(() => listar_transferencias(proveedor_uuid))
      .catch((e) => {
        console.error(e);
  ...
  render() {
    return html`
      <modal-generico .modalOpened=${this.openedModal} backurl='/proveedores' >
        <h4 slot="title">${this.proveedor.nombre}</h4>
        <div slot="menu">
          <vaadin-menu-bar
