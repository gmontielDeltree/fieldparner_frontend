...
import { borrar_tercero, listar_terceros } from "./terceros-funciones";
import { Tercero } from "../tipos/terceros";

@customElement("terceros-lista")
export class TercerosLista extends LitElement {
  @property()
  ...
  private menu_detalles_item = (item: Tercero) => [
    {
      component: createMenuDots("ellipsis-dots-v"),
      tooltip: get("mas"),
      children: [
        {
          text: get("editar"),
          callback: () => {
            Router.go("terceros/" + item.uuid + "/edit");
            console.log("edit");
          },
        },
        {
  ...
  render() {
    return html`
      <modal-generico .modalOpened=${this.openedModal} backurl="/">
        <h4 slot="title">${translate("terceros")}</h4>
        <div slot="menu" s>
          <vaadin-menu-bar
