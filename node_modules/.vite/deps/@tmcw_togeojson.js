...
function coord1(value) {
  return value.replace(removeSpace, "").split(",").map(parseFloat).filter((num) => !isNaN(num));
}
...
function kmlWithFolders(node) {
  ...
  function traverse(node2, pointer) {
    if (isElement(node2)) {
      switch (node2.tagName) {
        case "Placemark": {
          const placemark = getPlacemark(node2, styleMap);
          if (placemark) {
    ...
    if (node2.childNodes) {
      for (let i = 0; i < node2.childNodes.length; i++) {
        traverse(node2.childNodes[i], pointer);
      }
    }
  }
  traverse(node, tree);
