...
const get_lista_insumos = async (db: PouchDB.Database) => {
  let genericos: Insumo[] = await fetch("/insumos_genericos.json").then(
  ...
  let genericos_filtrados = genericos.filter(
    (insumo) => !ids_propios.includes(insumo._id)
  );
...
const download_lista_de_insumos = async (db: PouchDB.Database) => {
  let data = await fetch("/products.json").then((response) => response.json());
...
export type { Insumo, CultivoAplicacion };
