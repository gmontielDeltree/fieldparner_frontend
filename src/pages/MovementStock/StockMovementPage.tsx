...
export const StockMovementPage: React.FC = () => {
  ...
  const rows = useMemo(() => {
    return stockMovements.map(sm => {
      ...
      const supplyOrCrop = isCrop
        ? `${sm.crop?.descriptionEN || '-'}`
