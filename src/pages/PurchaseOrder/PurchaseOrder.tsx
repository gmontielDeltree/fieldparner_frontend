...
import { useBusiness, useForm, useSupply, userPurchaseOrder } from '../../hooks';
    ...
    >
      <Grid item xs={12} sm={4}>
        <Autocomplete
          ...
          disableClearable
          renderInput={params => <TextField {...params} label={'Insumo'} variant='outlined' />}
          fullWidth
          ListboxProps={{
            style: {
              maxHeight: 248,
      ...
      <Grid item xs={12} sm={2}>
        <TextField
          ...
          disabled
          // name="supplyAmount"
          value={Helper.parseDecimalPointToComaWithCurrency(
            Number(supplyAmount) * Number(unitPrice),
            '$',
            2,
          )}
          // onChange={handleInputChange}
      ...
      <Grid item xs={12} sm={1} display='flex' justifyContent='center'>
        <IconButton color='success' aria-label='add' size='small' onClick={() => onClickPlus()}>
          <AddIcon />
        </IconButton>
      </Grid>
    </Grid>
...
export const EditOrderRow = ({ order, editRowItem, deleteRowItem }: EditOrderRowProps) => {
  const {
    formulario: formValues,
    supplyAmount,
    unitPrice,
    handleInputChange,
  } = useForm<DetailPurchaseOrderItem>({ ...order });
  ...
  return (
    <ItemRow key={order._id}>
      ...
      <TableCellStyled align='center'>
        {Helper.parseDecimalPointToComaWithCurrency(totalPrice, '$', 2)}
      </TableCellStyled>
      <TableCellStyled align='center'>
        {isEdit ? (
          <>
            <IconButton color='primary' aria-label='save' onClick={handleSaveEdit}>
              <SaveIcon />
            </IconButton>
            <IconButton color='secondary' aria-label='cancel' onClick={handleCancelEdit}>
              <CloseIcon />
            </IconButton>
          </>
        ) : (
          <>
            ...
            <IconButton onClick={() => deleteRowItem(order)} style={{ fontSize: '1rem' }}>
              <DeleteIcon />
            </IconButton>
          </>
        )}
...
export const PurchaseOrderPage: React.FC = () => {
  ...
  const { isLoading, createPurchaseOrder, getPurchaseOrderByOrder, updatePurchaseOrder } =
    userPurchaseOrder();
  ...
  const subtotal = listItemToPurchase.reduce(
    (accumulator, { supplyAmount, unitPrice }) =>
      accumulator + Number(supplyAmount) * Number(unitPrice),
    0,
  );
  ...
  const onClickSave = async () => {
    if (listItemToPurchase.length) {
      ...
      if (formValues.nroOrder === '') await createPurchaseOrder(order, details);
      else await updatePurchaseOrder(order, details);
  ...
  useEffect(() => {
    ...
    if (order) getPurchaseOrder(order);
    else setFormValues(initialForm);
  }, [order]);

  return (
    <TemplateLayout key='new-purchase-order' viewMap={true} viewSelector={false}>
      ...
      <Paper variant='outlined' sx={{ my: { xs: 3, md: 3 }, p: { xs: 2, md: 3 } }}>
        ...
        <NewOrderRow key='new-item-row' supplies={supplies} addNewItem={addNewSupply} />
        <TableContainer
          key='purchase-orders'
          sx={{
            minHeight: '120px',
          ...
          component={Paper}
        >
          <DataTable key='datatable-orders' columns={columnsDepositSupply} isLoading={isLoading}>
        ...
        <Grid container alignItems='center' justifyContent='flex-end' sx={{ mt: 2, mb: 1 }}>
          <Grid spacing={1} container width='45%' alignItems='center'>
            ...
            <GridStyled item xs={8}>
              <Typography variant='body1' textAlign='end'>
                {' '}
                {Helper.parseDecimalPointToComaWithCurrency(subtotal, '$', 2)}
              </Typography>
            </GridStyled>
            <GridStyled item xs={12} display='flex' justifyContent='space-between'>
              ...
              />
              <Typography variant='body1' textAlign='end'>
                {Helper.parseDecimalPointToComaWithCurrency(taxValue, '$', 2)}
              </Typography>
            </GridStyled>
            <GridStyled item xs={12} display='flex' justifyContent='space-between'>
              <Typography
              ...
              />
              <Typography variant='body1' textAlign='end'>
                {Helper.parseDecimalPointToComaWithCurrency(otherValue, '$', 2)}
              </Typography>
            </GridStyled>
            <GridStyled item xs={4}>
              <Typography variant='body1' textAlign='start'>
            ...
            <GridStyled item xs={8}>
              <Typography variant='body1' textAlign='end'>
                {Helper.parseDecimalPointToComaWithCurrency(totalValue, '$', 2)}
              </Typography>
            </GridStyled>
          </Grid>
        </Grid>
