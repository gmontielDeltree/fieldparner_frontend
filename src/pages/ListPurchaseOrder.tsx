...
const styles = StyleSheet.create({
  ...
  tableColHeader: { flex: 1, borderStyle: 'solid', borderWidth: 1, backgroundColor: '#D3D3D3', padding: 5 },
      ...
      <View style={styles.table}>
        ...
        {details.map((d, index) => (
          <View style={styles.tableRow} key={index}>
            <View style={styles.tableCol}><Text style={styles.tableCell}>{d.supply?.name}</Text></View>
            <View style={styles.tableCol}><Text style={styles.tableCell}>{d.supply?.unitMeasurement}</Text></View>
            <View style={styles.tableCol}><Text style={styles.tableCell}>{d.supplyAmount}</Text></View>
            <View style={styles.tableCol}><Text style={styles.tableCell}>{Helper.parseDecimalPointToComaWithCurrency(d.unitPrice, "$", 2)}</Text></View>
            <View style={styles.tableCol}><Text style={styles.tableCell}>{Helper.parseDecimalPointToComaWithCurrency((Number(d.unitPrice) * Number(d.supplyAmount)), "$", 2)}</Text></View>
          </View>
        ))}
      </View>
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Box>
          ...
          <PDFDownloadLink
            document={<PurchaseOrderPDF purchaseOrder={params.row} details={params.row.details} />}
