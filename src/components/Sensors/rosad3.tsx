import React, { useEffect, useRef } from "react";
import * as d3 from "d3";
import "bootstrap/dist/css/bootstrap.min.css";

interface RosaVientosChartProps {
  data: any;
}

const puntosCardinales = [
  "N",
  "NNE",
  "NE",
  "ENE",
  "E",
  "ESE",
  "SE",
  "SSE",
  "S",
  "SSW",
  "SW",
  "WSW",
  "W",
  "WNW",
  "NW",
  "NNW",
];

const RosaVientosChart: React.FC<RosaVientosChartProps> = ({ data }) => {
  const chartRef = useRef<SVGSVGElement>(null);

  useEffect(() => {
    if (data) {
      chart();
    }
  }, [data]);

  const chart = () => {
    const svg = d3.select(chartRef.current);
    const width = +svg.attr("width");
    const height = +svg.attr("height");
    const margin = { top: 40, right: 40, bottom: 40, left: 40 };
    const innerRadius = 20;
    const chartWidth = width - margin.left - margin.right;
    const chartHeight = height - margin.top - margin.bottom;
    const outerRadius = Math.min(chartWidth, chartHeight) / 2;
    const g = svg
      .append("g")
      .attr("transform", `translate(${width / 2}, ${height / 2})`);

    const angle = d3.scaleLinear().range([0, 2 * Math.PI]);
    const radius = d3.scaleLinear().range([innerRadius, outerRadius]);
    const x = d3.scaleBand().range([0, 2 * Math.PI]).align(0);
    const y = d3.scaleLinear().range([innerRadius, outerRadius]);
    const z = d3
      .scaleOrdinal()
      .range([
        "#4242f4",
        "#42c5f4",
        "#42f4ce",
        "#42f456",
        "#adf442",
        "#f4e242",
        "#f4a142",
        "#f44242",
      ]);

    x.domain(
      data.map((d: any) => {
        return d.angle;
      })
    );

    y.domain([
      0,
      d3.max(data, (d: any) => d.total),
    ]);

    z.domain(data.columns.slice(1));

    // Extend the domain slightly to match the range of [0, 2π].
    angle.domain([0, d3.max(data, (_d: any, i: number) => i + 1)]);

    radius.domain([
      0,
      d3.max(data, (d: any) => d.y0 + d.y),
    ]);

    let angleOffset = -360.0 / data.length / 2.0;

    g.append("g")
      .selectAll("g")
      .data(d3.stack().keys(data.columns.slice(1))(data))
      .enter()
      .append("g")
      .attr("fill", (d: any) => z(d.key))
      .selectAll("path")
      .data((d: any) => d)
      .enter()
      .append("path")
      .attr(
        "d",
        d3
          .arc()
          .innerRadius((d: any) => y(d[0]))
          .outerRadius((d: any) => y(d[1]))
          .startAngle((d: any) => x(d.data.angle))
          .endAngle((d: any) => x(d.data.angle) + x.bandwidth())
          .padAngle(0.01)
          .padRadius(innerRadius)
      )
      .attr("transform", () => `rotate(${angleOffset})`);

    // Axis and Labels
    const label = g
      .append("g")
      .selectAll("g")
      .data(data)
      .enter()
      .append("g")
      .attr("text-anchor", "middle")
      .attr("transform", (d: any) => {
        return (
          `rotate(${(x(d.angle) + x.bandwidth() / 2) * 180 / Math.PI - (90 - angleOffset)})` +
          `translate(${outerRadius + 30}, 0)`
        );
      });

    label
      .append("text")
      .attr("transform", (d: any) => {
        return (x(d.angle) + x.bandwidth() / 2 + Math.PI / 2) % (2 * Math.PI) < Math.PI
          ? "rotate(90)translate(0,16)"
          : "rotate(-90)translate(0,-9)";
      })
      .text((d: any) => d.angle)
      .style("font-size", "14px");

    // Y-axis circles and labels
    const yAxis = g.append("g").attr("text-anchor", "middle");

    const yTick = yAxis
      .selectAll("g")
      .data(y.ticks(5).slice(1))
      .enter()
      .append("g");

    yTick
      .append("circle")
      .attr("fill", "none")
      .attr("stroke", "gray")
      .attr("stroke-dasharray", "4,4")
      .attr("r", y);

    yTick
      .append("text")
      .attr("y", (d: any) => -y(d))
      .attr("dy", "-0.35em")
      .attr("x", -10)
      .text(y.tickFormat(5, "s"))
      .style("font-size", "14px");

    // Legend
    const legend = g
      .append("g")
      .selectAll("g")
      .data(data.columns.slice(1).reverse())
      .enter()
      .append("g")
      .attr("transform", (d: any, i: number) => {
        return `translate(${outerRadius + 15}, ${-outerRadius + 40 + (i - (data.columns.length - 1) / 2) * 20})`;
      });

    legend.append("rect").attr("width", 18).attr("height", 18).attr("fill", z);

    legend
      .append("text")
      .attr("x", 24)
      .attr("y", 9)
      .attr("dy", "0.35em")
      .text((d: any) => d)
      .style("font-size", "10px");
  };

  return (
    <div>
      <svg ref={chartRef} className="mx-auto" style={{ display: "block" }} width="500" height="400"></svg>
    </div>
  );
};

export default RosaVientosChart;
