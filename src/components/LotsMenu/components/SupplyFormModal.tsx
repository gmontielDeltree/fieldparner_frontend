...
import { useCrops, useForm, useSupply } from "../../../hooks";
import {
  Button,
  Grid,
  Paper,
...
interface SupplyFormModalProps {
  onSupplyCreated?: (supply: Supply) => void;
}

export const SupplyFormModal: React.FC<SupplyFormModalProps> = ({ onSupplyCreated }) => {
  ...
  const addNewSupply = async () => {
    try {
      const newSupply = await createSupply(formulario);
      await uploadDocumentFile();
      
      // Notificar al componente padre que se creó un nuevo insumo
      if (onSupplyCreated && newSupply) {
