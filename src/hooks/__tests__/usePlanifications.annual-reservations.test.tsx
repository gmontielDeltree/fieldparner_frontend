...
const createPlanSupplyLine = (totalCantidad: number) => ({
  ...
  deposito: { _id: 'dep-1', description: 'Depósito 1' },
  depositoId: 'dep-1',
  ubicacion: '',
  nroLote: 'L1',
  ordenRetiro: null,
...
const initMemoryDb = (name: string) => new PouchDB(`${dbNamePrefix}-${name}`, { adapter: 'memory' })
...
const seedDatabases = async () => {
  await dbContext.numerators.createIndex({ index: { fields: ['accountId', 'numeratorType'] } } as any)
  await dbContext.depositSupplyOrder.createIndex({ index: { fields: ['accountId', 'order'] } } as any)
  await dbContext.stock.createIndex({ index: { fields: ['accountId', 'id', 'depositId', 'nroLot'] } } as any)
